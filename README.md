...
Python is general purpose, multi-paradigm, interpreted, garbage collected, dynamically typed language with comprehensive standard library, _scalar types_ - all immutable, int, float, None, bool, _control flow_ - if, else, elif, _loops_ - while, for, _relational operators_, _logical operators_, _in_ - membership operator, _is_ - identity operator, _str_ - immutable string type, _bytes_ - binary type, _modules_ - .py files, _ __name__ attribute _ - detect if file is imported or executed in shell, _python execution model_ - function name is bound to definition at runtime, _passing command line arguments_, _shebang_, _docstrings_, _object and type models_, _identity equality_ - defined on language level, _value equality_ - defined on type level, _all variables are references, not boxes_, _pass by reference_ - both arguments and return values, _positional and keyword arguments_, _strong, dynamic type system_, _4 levels scope - LEGB_ - local, enclosing, global, built in, _everything is an object_ - including scalar types, functions and modules, _list [...], tuple (...), ranges, dictionary {key: val}, set {...}_ - built in collections, _protocols_ - set of functions, interface, _try, except, raise, finally_ - exceptions, _exceptions are part of the function signature and protocols_, _EAFP_ - easier to ask forgiveness than permission - try catch, can not be ignored, _LBYL_ - look before you leap - if else without exception, error codes, silent, _comprehensions_ - concise syntax to transform collections without side effects, _generator functions_ - iterables defined via functions, yield keyword, evaluated lazily, can model infinite sequences, _iteration protocols_ - iter() and next(), _generator expressions_ - create generator objects  with comprehensions, _classes_ - custom types, _self_ - this, _ __init__() _ - initializer, for configuring already created object, _methods_ - first argument is self and passed explicitly, _no access modifiers_ - everything is public, _invariants_ - constraints, _duck typing_ - argument fitness determined at runtime, _late binding_ - use object to resolve method calling at runtime, _polymorphism_ - through late binding, not through inheritance, _inheritance_ - used only for reusing implementation, _files and file like objects_ - text and binary mode, support iterator and context manager protocols, _universal newline_ - OS independent, _with block_, _packages_ - folders, _modules_ - files, _sys.path_ - list of all included directories with modules, _PYTHONPATH_ - environment variable included in sys.path, _ __init__.py_, _subpackages_, _relative imports_, _namespace packages_, _executable directories_, _recommended layout_ - recommended folder structure, _module singletons_, _positional and keyword arguments_, _default arguments_, _callable instances_, _callables_ - functions, classes, callable objects, lambdas, _conditional expressions_ - ternary expression, _lambdas_ - single expression anonymous functions, _extended formal and actual arguments syntax_ - def fn(*args, **kwargs), fn(*args), fn(**args), _unpacking_ - destructuring, _global functions_, _local functions_, _closures_ - maintain references from parent scopes, _function factories_ - functions that returns new specialized functions, _nonlocal and global keywords_, _decorators_ - syntactic sugar for higher oder functions, accepts callable and returns callable, _@functools.wraps()_ - forwarding metadata of the decorated function, _instance attributes_, _class attributes_, _@staticmethod_, _@classmethod_, _@property_ - property getter, _@name.setter_ - property setter, _str()_ - simple string representation, _repr()_ - verbose representation, _format()_, _ascii(), ord(), chr()_ - ascii, unicode helpers, _int, float, Decimal, Fraction, complex()_ - number types, _abs(), round()_ - math helpers, _bin(), oct(), hex(), int()_ - number base conversion, _datetime module_ - time(), datetime(), timedelta(), timezone(), _advanced comprehensions_, _map(), filter(), reduce()_, _iter(iterable)_ - iterable protocol, _next(iterator)_ - iterator protocol, _single inheritance_, _multiple inheritance_, _isinstance(), issubclass()_, _method resolution order - MRO_ - search order of inheritance graph, _super()_, _inheritance usage_ - for reusing implementation, _collection protocols_ - container, sized, iterable, sequence, set, string, value equality protocols, _collections.abc_ - module with base implementations for collection protocols, _standard exception hierarchy_, _custom exceptions_, _implicit chaining exceptions_, _explicit chaining exceptions_, _traceback objects_, _asserts_, _context manager_, _context manager protocol_ - __enter__(), __exit__(), _@contextlib.contextmanager_ - decorator for creating context managers, _introspection_ - programs can inspect their own structure and state, types, objects, scopes, _while else, for else clauses_, _try else clause_, _emulating switch statements_, _bitwise operators_, _bin(), to_bytes()_, _bytes type_ - immutable sequence of bytes, _bytearray type_ - mutable sequence of bytes, _reading C structures_, _memoryview, memory mapped files_, 
...
_list [...], tuple (...), ranges, dictionary {key: val}, set {...}_ - built in collections, _protocols_ - set of functions, interface, 

_try, except, raise, finally_ - exceptions, _exceptions are part of the function signature and protocols_, _EAFP_ - easier to ask forgiveness than permission - try catch, can not be ignored, _LBYL_ - look before you leap - if else without exception, error codes, silent,

_comprehensions_ - concise syntax to transform collections without side effects, _generator functions_ - iterables defined via functions, yield keyword, evaluated lazily, can model infinite sequences, _iteration protocols_ - iter() and next(), _generator expressions_ - create generator objects  with comprehensions,

_classes_ - custom types, _self_ - this, _ __init__() _ - initializer, for configuring already created object, _methods_ - first argument is self and passed explicitly, _no access modifiers_ - everything is public, _invariants_ - constraints, _duck typing_ - argument fitness determined at runtime, _late binding_ - use object to resolve method calling at runtime, _polymorphism_ - through late binding, not through inheritance, _inheritance_ - used only for reusing implementation, 

_files and file like objects_ - text and binary mode, support iterator and context manager protocols, _universal newline_ - OS independent, _with block_,

---

_packages_ - folders, _modules_ - files, _sys.path_ - list of all included directories with modules, _PYTHONPATH_ - environment variable included in sys.path, _ __init__.py_, _subpackages_, _relative imports_, _namespace packages_, _executable directories_, _recommended layout_ - recommended folder structure, _module singletons_, 

_positional and keyword arguments_, _default arguments_, _callable instances_, _callables_ - functions, classes, callable objects, lambdas, _conditional expressions_ - ternary expression, _lambdas_ - single expression anonymous functions, _extended formal and actual arguments syntax_ - def fn(*args, **kwargs), fn(*args), fn(**args), _unpacking_ - destructuring, _global functions_, _local functions_, _closures_ - maintain references from parent scopes, 

_function factories_ - functions that returns new specialized functions, _nonlocal and global keywords_, _decorators_ - syntactic sugar for higher oder functions, accepts callable and returns callable, _@functools.wraps()_ - forwarding metadata of the decorated function, 

_instance attributes_, _class attributes_, _@staticmethod_, _@classmethod_, _@property_ - property getter, _@name.setter_ - property setter, 

_str()_ - simple string representation, _repr()_ - verbose representation, _format()_, _ascii(), ord(), chr()_ - ascii, unicode helpers, 

_int, float, Decimal, Fraction, complex()_ - number types, _abs(), round()_ - math helpers, _bin(), oct(), hex(), int()_ - number base conversion, _datetime module_ - time(), datetime(), timedelta(), timezone(), 

_advanced comprehensions_, _map(), filter(), reduce()_, _iter(iterable)_ - iterable protocol, _next(iterator)_ - iterator protocol,

_single inheritance_, _multiple inheritance_, _isinstance(), issubclass()_, _method resolution order - MRO_ - search order of inheritance graph, _super()_, _inheritance usage_ - for reusing implementation, 

_collection protocols_ - container, sized, iterable, sequence, set, string, value equality protocols, _collections.abc_ - module with base implementations for collection protocols, 

_standard exception hierarchy_, _custom exceptions_, _implicit chaining exceptions_, _explicit chaining exceptions_, _traceback objects_, _asserts_, 

_context manager_, _context manager protocol_ - __enter__(), __exit__(), _@contextlib.contextmanager_ - decorator for creating context managers, 

_introspection_ - programs can inspect their own structure and state, types, objects, scopes,

_while else, for else clauses_, _try else clause_, _emulating switch statements_, 

_bitwise operators_, _bin(), to_bytes()_, _bytes type_ - immutable sequence of bytes, _bytearray type_ - mutable sequence of bytes, _reading C structures_, _memoryview, memory mapped files_, 
...
_ __closure__ _ - reference to the closure variables, _function factories_ - functions that returns new specialized functions, _nonlocal and global keywords_ - reuse variables frm parent and global scope, _decorators_ - syntactic sugar for higher oder functions, accepts callable and returns callable, can be nested, _@functools.wraps()_ - helper decorator for forwarding metadata of the decorated function, 

#### Properties and Class Methods

_instance attributes_ - per instance, _class attributes_ - per class, same for all instances, _@staticmethod_ - does not reference neither class nor self, _@classmethod_ - references only class, _@property_ - property getter, _@name.setter_ - property setter, 
